...
class Wooecpay_Gateway_Webatm extends Wooecpay_Gateway_Base
{
    ...
    {
        $this->id                   = 'Wooecpay_Gateway_Webatm';
        $this->payment_type         = 'WebATM';
        $this->icon                 = plugins_url('images/icon.png', dirname(dirname( __FILE__ )) );
        $this->has_fields           = false;
        $this->method_title         = '綠界網路ATM';
        ...
        $this->form_fields          = include WOOECPAY_PLUGIN_INCLUDE_DIR . '/config/payment/settings-gateway-webatm.php' ;
        $this->init_settings();
    ...
    public function process_payment($order_id)
    {
        ...
        $order->add_order_note(__('Pay via ECPay Web ATM', 'ecpay-ecommerce-for-woocommerce'));
